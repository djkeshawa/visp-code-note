...
/*
 * There was a `taskFiltersNeedNote` here, for a caller that wanted to know whether a facet
 * obliged it to build a uri->note map. Its only caller now builds that map for every query that
 * has tasks in it — the search's last tie-break is recency, and a task's date is its note's — so
 * the question no longer has an answer that changes anything. Removed rather than left exported
 * and unused, which is how a narrowing that no longer narrows gets called again.
 */
